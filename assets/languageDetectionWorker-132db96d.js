var _a;
import { D as Disposable, ab as LRUCache, w as isWeb, qQ as FileAccess, br as Schemas, gu as __decorate, gv as __param, au as EditorWorkerClient, D1 as SimpleWorkerClient, D2 as EditorWorkerHost, a2 as URI, D3 as LanguageDetectionStatsId, vc as nodeModulesPath, vd as nodeModulesAsarPath, hR as IWorkbenchEnvironmentService, bb as ILanguageService, ay as IConfigurationService, D4 as IDiagnosticsService, d7 as IWorkspaceContextService, a4 as IModelService, gx as IEditorService, cu as ITelemetryService, aa as IStorageService, fM as ILogService, at as ILanguageConfigurationService, r5 as registerAssets, pD as ILanguageDetectionService, hh as SyncDescriptor } from "./theme-defaults-7d2b2d38.js";
...
let LanguageDetectionService = (_a = class extends Disposable {
  constructor(_environmentService, languageService, _configurationService, _diagnosticsService, _workspaceContextService, modelService, _editorService, telemetryService, storageService, _logService, languageConfigurationService) {
    ...
    this._languageDetectionWorkerClient = new LanguageDetectionWorkerClient(
      ...
      languageService,
      telemetryService,
      this._environmentService.isBuilt && !isWeb ? FileAccess.asBrowserUri(`${moduleLocationAsar}/dist/lib/index.js`).toString(true) : FileAccess.asBrowserUri(`${moduleLocation}/dist/lib/index.js`).toString(true),
      this._environmentService.isBuilt && !isWeb ? FileAccess.asBrowserUri(`${moduleLocationAsar}/model/model.json`).toString(true) : FileAccess.asBrowserUri(`${moduleLocation}/model/model.json`).toString(true),
      this._environmentService.isBuilt && !isWeb ? FileAccess.asBrowserUri(`${moduleLocationAsar}/model/group1-shard1of1.bin`).toString(true) : FileAccess.asBrowserUri(`${moduleLocation}/model/group1-shard1of1.bin`).toString(true),
      this._environmentService.isBuilt && !isWeb ? FileAccess.asBrowserUri(`${regexpModuleLocationAsar}/dist/index.js`).toString(true) : FileAccess.asBrowserUri(`${regexpModuleLocation}/dist/index.js`).toString(true),
  ...
  isEnabledForLanguage(languageId) {
    return !!languageId && this._configurationService.getValue(_a.enablementSettingKey, { overrideIdentifier: languageId });
  }
  getLanguageBiases() {
    if (!this.dirtyBiases) {
  ...
  async detectLanguage(resource, supportedLangs) {
    const useHistory = this._configurationService.getValue(_a.historyBasedEnablementConfig);
    const preferHistory = this._configurationService.getValue(_a.preferHistoryConfig);
    if (useHistory) {
  ...
  initEditorOpenedListeners(storageService) {
    try {
      const globalLangHistoryData = JSON.parse(storageService.get(_a.globalOpenedLanguagesStorageKey, 0, "[]"));
      this.historicalGlobalOpenedLanguageIds.fromJSON(globalLangHistoryData);
    } catch (e) {
      console.error(e);
    }
    try {
      const workspaceLangHistoryData = JSON.parse(storageService.get(_a.workspaceOpenedLanguagesStorageKey, 1, "[]"));
    ...
    this._register(this._editorService.onDidActiveEditorChange(() => {
      ...
      if (activeLanguage && ((_b = (_a2 = this._editorService.activeEditor) == null ? void 0 : _a2.resource) == null ? void 0 : _b.scheme) !== Schemas.untitled) {
        ...
        storageService.store(_a.globalOpenedLanguagesStorageKey, JSON.stringify(this.historicalGlobalOpenedLanguageIds.toJSON()), 0, 1);
        storageService.store(_a.workspaceOpenedLanguagesStorageKey, JSON.stringify(this.historicalWorkspaceOpenedLanguageIds.toJSON()), 1, 1);
        this.dirtyBiases = true;
      }
    }));
  }
}, _a.enablementSettingKey = "workbench.editor.languageDetection", _a.historyBasedEnablementConfig = "workbench.editor.historyBasedLanguageDetection", _a.preferHistoryConfig = "workbench.editor.preferHistoryBasedLanguageDetection", _a.workspaceOpenedLanguagesStorageKey = "workbench.editor.languageDetectionOpenedLanguages.workspace", _a.globalOpenedLanguagesStorageKey = "workbench.editor.languageDetectionOpenedLanguages.global", _a);
...
registerAssets({
  "vs/../../node_modules/@vscode/vscode-languagedetection/model/model.json": new URL("/statetree-language/assets/model-100ce176.json", self.location).href,
  "vs/../../node_modules/@vscode/vscode-languagedetection/model/group1-shard1of1.bin": new URL("/statetree-language/assets/group1-shard1of1-fab64426.bin", self.location).href
});
